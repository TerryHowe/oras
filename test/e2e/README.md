...
The e2e testing infrastructure deploys two OCI-compliant registries to a Kubernetes cluster:

1. **Docker Registry v2** - The standard OCI distribution registry
2. **Zot Registry** - A modern, lightweight OCI registry with additional features
...
- Deploy Docker Registry v2 and Zot Registry
- Set up persistent storage for both registries
...
### Access Registries Locally (Alternative Testing Method)

To access the registries from your local machine for manual testing or running tests locally:

```bash
./test/e2e/scripts/port-forward.sh
```

This sets up port forwarding:
- **Docker Registry**: `localhost:5000`
- **Zot Registry**: `localhost:5001`

Press `Ctrl+C` to stop port forwarding.

With port forwarding active, you can run tests locally:
...
└── scripts/               # Management scripts
    ...
    ├── port-forward.sh    # Setup local port forwarding
...
kubectl describe pvc docker-registry-pvc -n oras-e2e-tests
kubectl describe pvc zot-registry-pvc -n oras-e2e-tests
```

### Port forwarding not working

1. Ensure the pods are running: `./test/e2e/scripts/status.sh`
2. Check if ports 5000 and 5001 are already in use on your machine
3. Try stopping and restarting port forwarding
...
### Alternative Approach (Local Testing with Port Forwarding)

If you prefer to run tests locally with port forwarding:
