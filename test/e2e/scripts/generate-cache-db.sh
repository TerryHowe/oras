...
if ! kubectl exec "$POD_NAME" -n oras-e2e-tests -- test -f /etc/zot/cache.db; then
    echo "Error: cache.db not found in /etc/zot/"
    echo "Listing contents of /etc/zot:"
    kubectl exec "$POD_NAME" -n oras-e2e-tests -- ls -la /etc/zot/
...
kubectl exec "$POD_NAME" -n oras-e2e-tests -- ls -lh /etc/zot/cache.db

# Copy cache.db from pod to local
echo "Copying cache.db to testdata directory..."
kubectl cp "oras-e2e-tests/$POD_NAME:/etc/zot/cache.db" "$TESTDATA_DIR/cache.db"
